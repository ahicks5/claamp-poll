"""
Migration script to fix spread_polls unique constraint.

This script:
1. Drops old constraint uq_spread_poll_season_week (season, week only)
2. Creates new constraint uq_spread_poll_group_season_week (group_id, season, week)

This allows multiple groups to have spread polls for the same season/week.

Run this to fix the constraint that prevents multiple groups from having spreads.
"""
...
def get_existing_constraints():
    """Get all constraints on spread_polls table"""
    inspector = inspect(engine)

    # Check if table exists
    tables = inspector.get_table_names()
    if 'spread_polls' not in tables:
        return None, []

    # Get unique constraints
    unique_constraints = inspector.get_unique_constraints('spread_polls')
...
def run_migration():
    ...
    try:
        print("\n" + "="*60)
        print("SPREAD POLLS CONSTRAINT MIGRATION")
        print("="*60 + "\n")
        ...
        if unique_constraints is None:
            print("[!] spread_polls table does not exist!")
        ...
        for constraint in unique_constraints:
            ...
            if constraint['name'] == 'uq_spread_poll_season_week':
                old_constraint_exists = True
            if constraint['name'] == 'uq_spread_poll_group_season_week':
        ...
        if old_constraint_exists:
            try:
                session.execute(text("""
                    ALTER TABLE spread_polls
                    DROP CONSTRAINT uq_spread_poll_season_week
                """))
                session.commit()
                print("  ✓ Dropped old constraint: uq_spread_poll_season_week")
        ...
        if not new_constraint_exists:
            try:
                session.execute(text("""
                    ALTER TABLE spread_polls
                    ADD CONSTRAINT uq_spread_poll_group_season_week
                    UNIQUE (group_id, season, week)
                """))
                session.commit()
                print("  ✓ Created new constraint: uq_spread_poll_group_season_week")
            except Exception as e:
        ...
        print("  Current constraints on spread_polls:")
        ...
        print("="*60)
        print("\nYou can now create spread polls for multiple groups")
...
if __name__ == "__main__":
    print("\nThis migration will fix the spread_polls constraint to support multiple groups.")
